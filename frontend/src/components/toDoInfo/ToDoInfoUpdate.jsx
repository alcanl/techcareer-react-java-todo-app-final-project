...
// BLOG CATEGORY API
import BlogCategoryApi from "../../services/ToDoInfoApi";

// FUNcTION BLOG CATEGORY UPDATE
function ToDoInfoUpdate({ t, i18n, props }) {
  // REDIRECT
  ...
  useEffect(() => {
    ...
    // FIND BLOG CATEGORY
    BlogCategoryApi.toDoApiFindById(blogCategoryUpdateParamsId.id)
      .then((response) => {
  ...
  const BlogCategoryUpdateSubmit = async (event) => {
    ...
    // API
    try {
      const response = await BlogCategoryApi.toDoApiUpdate(blogCategoryUpdateParamsId.id,blogCategoryUpdateObject);
      if (response.status === 200) {
        // Spinner Pasif et
        setSpinner(false);
...
// Export i18n Wrapper
export default withTranslation()(ToDoInfoUpdate);
